...
救援模式遍历所有带成对 bulk 端点的接口，每个都尝试 `AT` / `ATI` 等多种指令、
DTR 拉高与不拉高两种状态，共三轮，轮间等待 8 秒以应对模块尚未启动完成。
过程实时输出日志。

**这是本项目相比 PC 端工具链的一个实际优势**：不依赖任何内核驱动，
不需要 `new_id`，插上手机就能把配置改回来。
...
实际是主机端写死了原来的串口号（如 `/dev/ttyUSB2`），换个口就能通。
本 App 采用接口遍历探测，通常可自动恢复。
...
- 救援模式只能处理「AT 口移位」这一类问题。如果用 `AT+QCFG="usbcfg",...,0,0,0,0,0,0,0`
